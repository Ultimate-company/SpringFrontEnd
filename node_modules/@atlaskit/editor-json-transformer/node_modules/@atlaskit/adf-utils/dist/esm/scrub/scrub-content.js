...
export var scrubStr = function scrubStr(val) {
  ...
  return _toConsumableArray(val).map(function (char, index, chars) {
...
export var scrubLink = function scrubLink(marks, _ref3) {
...
export var scrubAttrs = function scrubAttrs(nodeType, attrs) {
  ...
  if (_typeof(attrsObj) === 'object' && !Array.isArray(attrsObj)) {
    return scrubObj(nodeType, attrsObj);
  }
  if (Array.isArray(attrsObj)) {
    return attrsObj.map(function (el) {
      return _typeof(el) === 'object' ? scrubObj(nodeType, el) : scrubAttrs(nodeType, el);
    });
  }
  throw new TypeError("scrubAttrs: encountered unsupported attributes type \"".concat(_typeof(attrsObj), "\"\n    of value ").concat(JSON.stringify(attrsObj)));
};